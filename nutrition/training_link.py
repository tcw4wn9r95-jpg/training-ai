...
Reads the training app's JSON files (which live in the repo root, one level up
from nutrition/) so the meal planner can fuel training days, adapt to sleep, and
share the athlete's profile/goals. Body weight stays single-sourced in NutriPrep
(users/<m>/weight_log.json) — the training app reads it from there.

All reads are best-effort: if the training app isn't present, every helper
returns an empty/neutral result and meal generation proceeds unchanged.
"""
import json
...
def load_training_week(root: Path, week_dates: dict) -> dict:
    """
    ...
    """
    plan = _load(root, "weekly_plan.json", [])
